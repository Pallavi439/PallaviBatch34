...
Feature: When Location Geofencing is created only for Locality then it will be taken under consideration for distance measure
  ...
  Scenario: Set session data for scenario
    * user generate random value "${wh3-se1}" and store into session "sales_person_mobile"
    * user generate random value "${wh3-sp1.locality}" and store into session "sales_person_locality"
    * user generate random value "${wh3-customer-1-title}" and store into session "sales_customer_name"
    * user generate random value "${wh3-sales-person-1-name}" and store into session "sales_person_name"
    * user generate random value "${wh3-sp1.locality}" and store into session "geofencing_locality_name"
    * user generate random value "${wh3-sp1.locality}" and store into session "geofencing_location_name"
  ...
  Scenario: Create Locality Geofencing if not available
    * user generate random value "Locality" and store into session "geo_link_type"
    * user generate random value "1000" and store into session "geo_distance"
    * user create "Location Geofencing" by api
      | link_name | ${geofencing_locality_name} | /location_geofencing.json |
    * response status code should be 200

    * user set value by frappe client set value api with filters and fieldname
      | Location Geofencing | ${Location Geofencing.name} | outlet_geofencing | ${geo_distance} |
  ...
  Scenario: Set session data for scenario
    ...
    * user generate random value "${wh2-sp1.locality}" and store into session "geofencing_location_name"

  Scenario: Create Locality Geofencing if not available
    * user generate random value "Locality" and store into session "geo_link_type"
    * user generate random value "800" and store into session "geo_distance"
    * user create "Location Geofencing" by api
      | link_name | ${geofencing_locality_name} | /location_geofencing.json |
    * response status code should be 200
