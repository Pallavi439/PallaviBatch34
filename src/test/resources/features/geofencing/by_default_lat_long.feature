@gajender.singh@elastic.run @default_geofencing @regression2
Feature: When all config regarding geofencing are not available in the warehouse,territory,locality and withrun app config
  ...
  Scenario: Set session data for scenario
    ...
    * user generate random value "${wh2-territory-1}" and store into session "sales_person_territory"
    * user generate random value "${warehouse-2}" and store into session "sales_person_warehouse"
  ...
  Scenario:Set locality geofencing to 0
    ...
    * user generate random value "0" and store into session "geo_distance"
    * user create "Location Geofencing" by api
      | link_name | ${sales_person_locality} | /location_geofencing.json |
    * response status code should be 200
  ...
  Scenario: Set territory geofencing to zero
    * user generate random value "Territory" and store into session "geo_link_type"
    * user generate random value "0" and store into session "geo_distance"
    * user generate random value "${wh2-territory-1}" and store into session "geofencing_location_name"

    * user create "Location Geofencing" by api
      | link_name | ${sales_person_territory} | /location_geofencing.json |
    * response status code should be 200
  ...
  Scenario: Set Warehouse Outlet geofencing to 0
    ...
    * response status code should be 200
    * user set value by frappe client set value api with filters and fieldname
      | Warehouse | ${wh_fieldname} | outlet_geofencing | 0 |
    * response status code should be 200
